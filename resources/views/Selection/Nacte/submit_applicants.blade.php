...
<div class="row">
    <div class="card">
        <div class="card-body">
            <form action="{{ route('submitted-applicants.submit') }}" method="GET">
                @csrf
                <div class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-info" id="runselection">Fetch List</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    @if ((isset($seclectedApplicants) && $seclectedApplicants->isNotEmpty()))
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h6 class="mb-0">Applicants Who Meet the Criteria </h6>
            <div class="mb-3">
                <label for="num-rows" class="form-label">Number of Rows to Select:</label>
                <input type="number" id="num-rows" class="form-control" min="1" max="{{ count($seclectedApplicants)}}" placeholder="Enter number of rows" />
            </div>
                ...
                <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Keen Mobile</th>
                            <th>Keen Domicile</th>
                            <th>Keen Nationality</th>
                            <th>Keen Relationship</th>
                            <th>Course</th>
                            <th>More</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($seclectedApplicants as $applicant)
                        <tr>
                            <td>
                                <div class="form-check">
                            ...
                            <td>{{ ucfirst(strtolower($applicant->first_name)) }} {{ ucfirst(strtolower($applicant->last_name)) }}</td>
                            ...
                            <td>{{  ucfirst(strtolower($applicant->next_kin_name)) }}</td>
                            <td>{{  $applicant->next_kin_email }}</td>
                            <td>{{  $applicant->next_kin_address }}</td>
                            <td>{{  $applicant->next_kin_phone }}</td>
                            <td>{{  $applicant->next_kin_region }} {{  $applicant->next_kin_district }}</td>
                            <td>{{  $applicant->next_kin_nationality }}</td>
                            <td>{{  $applicant->next_kin_relationship }}</td>
                            <td>
                                @php
                                    $programme = \App\Models\Programme::find($applicant->iaa_programme_code);
                                @endphp
