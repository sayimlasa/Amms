...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h6 class="mb-0">Submited Applications</h6>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($applicants as $applicant)
                            <tr data-entry-id="{{ $applicant->id }}">
                                ...
                                <td>{{ trim("$applicant->fname $applicant->mname $applicant->lname") }}</td>
                                ...
                                <td>
                                    @php
                                        $completed = \App\Models\ApplicantsChoice::where(
                                            'applicant_user_id',
                                            $applicant->applicant_user_id,
                                        )->where('index_no',$applicant->index_no)->first();
                                    @endphp
                                    {{ $completed?->programmeChoice1->name ?? 'N/A' }}-{{ $completed?->programmeChoice1->short ?? 'N/A' }}
                                </td>
                                <td>
                                    <a class="dropdown-item view-item-btn"
                                        href="{{ route('detailed-applicant-info', ['applicant_user_id' => $applicant->applicant_user_id, 'index_no' => $applicant->index_no]) }}">
