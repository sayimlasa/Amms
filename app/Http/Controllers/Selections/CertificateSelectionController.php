...
class CertificateSelectionController extends Controller
{
    ...
    //     // Get the programme and category IDs related to the level
    //     $programme = Programme::where('application_level_id', $level->id)->get();
    //     $selectedDiplomaApplicants = collect();
    //     return view('Admission.Applicants.selections.index-certificate', compact('programme', 'selectedDiplomaApplicants'));
    // }
    // public function indexFour(Request $request)
    // {
    //     // Get Application Level where name contains 'Certificate'
    //     $level = ApplicationLevel::where('name', 'LIKE', '%Certificate%')->first();
    ...
    //     // Get the active Academic Year, Intake, and Application Window
    //     $academicYear = AcademicYear::where('active', 1)->first();
    //     $intake = Intake::where('active', 1)->first();
    //     $window = ApplicationWindow::where('active', 1)->first();
    ...
    //     if (!$academicYear || !$intake || !$window) {
    //         return redirect()->back()->withErrors('Required records (Academic Year, Intake, or Window) are missing or inactive.');
    //     }
    
    //     $programme_id = (int) $request->input('programme_id');
        
    //     // Get Programme and Category IDs related to the application level
    //     $programme = Programme::where('application_level_id', $level->id)->get();
    ...
    //     // Get applicants based on selected programme
    //     $applicantUsers = ApplicantsChoice::where(function ($query) use ($programme_id) {
    //             $query->where('choice1', '=', $programme_id)
    //                 ->orWhere('choice2', '=', $programme_id)
    //                 ->orWhere('choice3', '=', $programme_id);
    //         })
    //         ->where('status', 0)
    ...
    //     // Initialize a list for eligible applicants
    //     $eligibleApplicants = collect();
    
    //     // Loop through each applicant and check their Form 4 results
    //     foreach ($applicantUsers as $applicantId) {
    //         $excludedSubjects = ['Islamic Studies', 'Biblical Knowledge'];  // Exclude specific subjects
    
    //         // Fetch Form 4 results for each applicant
    //         $form4Results = Form4Result::where('applicant_user_id', $applicantId)
    //             ->whereNotIn('subject_name', $excludedSubjects)
    //             ->whereIn('grade', ['A', 'B+', 'B', 'C', 'D'])
    //             ->get();
    
    //         // Count the number of passing grades (A, B+, B, C, D)
    //         $passingGradesSubjects = $form4Results->filter(function ($result) {
    //             return in_array($result->grade, ['A', 'B+', 'B', 'C', 'D']);
    //         });
    
    //         // If the applicant has at least 4 passing grades, consider them eligible
    //         if ($passingGradesSubjects->count() >= 4) {
    //             $eligibleApplicants->push($applicantId);
    //         }
    //     }
    
    //     // Fetch diploma certificate applicants based on eligibility
    //     $selectedDiplomaApplicants = SelectedDiplomaCertificate::list(
    //         $programme_id ?? null,
    //         $intake->id,
    //         $window->id,
    //         $academicYear->id,
    //         $categoryIds
    //     )->whereIn('id', $eligibleApplicants->toArray());
    
    //     // Log the count of eligible applicants
    //     Log::info("Total number of eligible applicants: " . $eligibleApplicants->count());
    
    //     // Pass data to view (avoid passing unnecessary logs to the view)
    //     return view('Admission.Applicants.selections.index-certificate', compact('programme', 'selectedDiplomaApplicants'));
    // }
