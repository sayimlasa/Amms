...
class SubmitedApplicationsController extends Controller
{
    public function allCampuses()
    {
        ...
        $applicants = ApplicantsInfo::with(
            ...
            ->whereIn('index_no', $completed->pluck('index_no')) // Use whereIn for an array of index numbers
    ...
    public function arushaCampus()
    {
        ...
        $otherEmployers = Employer::where('name', 'like', '%other%')->pluck('id');
        ...
        $applicants = ApplicantsInfo::with(
            ...
            ->where('acadmic_year_id', $academicYear) // Filter by academic year
            ->where(function ($query) use ($excludedEmployers, $otherEmployers) {
                $query->whereNull('employer_id')
                    ->orWhereNotIn('employer_id', $excludedEmployers)
                    ->orWhereIn('employer_id', $otherEmployers);
            })->get();

        // Return the view with the applicants data
        return view('Admission.Applicants.submited-applications.partial_info', compact('applicants','completed'));
    }
    public function babatiCampus()
    {
        ...
        // Get the Arusha campus ID
        $arushacampus = Campus::where('name', 'like', '%babati%')->pluck('id')->first(); // Ensure we get the first ID value
    ...
    public function dodomaCampus()
    {
        ...
        // Get the Arusha campus ID
        $arushacampus = Campus::where('name', 'like', '%dodoma%')->pluck('id')->first(); // Ensure we get the first ID value
    ...
    public function darCampus()
    {
        ...
        // Get the Arusha campus ID
        $arushacampus = Campus::where('name', 'like', '%dar%')->pluck('id')->first(); // Ensure we get the first ID value
